...
def create_model(opt):
    if opt['mode'] == 'sr':
        ...
        raise NotImplementedError(
            "The mode [%s] of networks is not recognized." % opt['mode'])
...
def define_net(opt):
    ...
    if which_model == 'DBPN':
        ...
        net = SRFBN(in_channels=opt['in_channels'],
                    out_channels=opt['out_channels'],
                    num_features=opt['num_features'],
        ...
        net = GMFN(in_channels=opt['in_channels'],
                   out_channels=opt['out_channels'],
                   num_features=opt['num_features'],
                   num_steps=opt['num_steps'],
                   num_blocks=opt['num_blocks'],
                   num_reroute_feats=opt['num_reroute_feats'],
                   num_refine_feats=opt['num_refine_feats'],
                   upscale_factor=opt['scale'])

    elif which_model.find('RDN') >= 0:
        from .rdn_arch import RDN
        net = RDN(in_channels=opt['in_channels'],
                  out_channels=opt['out_channels'],
                  num_features=opt['num_features'],
                  num_blocks=opt['num_blocks'],
        ...
        net = EDSR(in_channels=opt['in_channels'],
                   out_channels=opt['out_channels'],
                   num_features=opt['num_features'],
                   num_blocks=opt['num_blocks'],
        ...
        raise NotImplementedError(
            "Network [%s] is not recognized." % which_model)
