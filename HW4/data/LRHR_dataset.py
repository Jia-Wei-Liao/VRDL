...
class LRHRDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.repeat = 2
        self.paths_HR = common.get_image_paths(
            self.opt['data_type'], self.opt['dataroot_HR'])
        self.paths_LR = common.get_image_paths(
            self.opt['data_type'], self.opt['dataroot_LR'])

        assert self.paths_HR, '[Error] HR paths are empty.'

        if self.paths_LR and self.paths_HR:
            assert len(self.paths_LR) == len(self.paths_HR), \
                '[Error] HR and LR have different number of images.'
